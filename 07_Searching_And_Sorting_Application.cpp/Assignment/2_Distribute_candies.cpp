/* Distribute Candies
Send Feedback
Shaky has N (1<=N<=50000) candy boxes each of them contains a non-zero number of candies (between 1 and 1000000000). Shaky want to distibute these candies among his K (1<=K<=1000000000) IIIT-Delhi students. He want to distibute them in a way such that:
1. All students get equal number of candies.
2. All the candies which a student get must be from a single box only.
As he want to make all of them happy so he want to give as many candies as possible. Help Shaky in finding out what is the maximum number of candies which a student can get.
Input
First line contains 1<=T<=20 the number of test cases. Then T test cases follow. First line of each test case contains N and K. Next line contains N integers, ith of which is the number of candies in ith box.
Output
For each test case print the required answer in a seperate line.
Sample Input:
2
3 2
3 1 4
4 1
3 2 3 9
Sample Output:
3
9 */

#include <bits/stdc++.h>
using namespace std;
#define int long long
bool check(int *arr, int numberOfBoxes, int numberOfStudents, int mid)
{
    int count = 0;
    for (int i = 0; i < numberOfBoxes; i++)
    {
        count += arr[i] / mid;
        if (count >= numberOfStudents)
        {
            return true;
            // cout << "possible for mid " << mid << endl;
        }
        // cout << "not possible for mid " << mid << endl;
    }
    return false;
}

int distribute(int *arr, int numberOfBoxes, int numberOfStudents)
{
    int ans = 0;
    sort(arr, arr + numberOfBoxes);
    int start = 1;
    int end = arr[numberOfBoxes - 1];

    while (start <= end)
    {
        int mid = (start + end) / 2;
        if (check(arr, numberOfBoxes, numberOfStudents, mid))
        {
            ans = max(ans, mid);
            start = mid + 1;
            // cout << "possible for mid " << mid << endl;
        }
        else
        {
            // cout << "not possible for mid " << mid << endl;
            end = mid - 1;
        }
    }
    // cout << endl;

    return ans;
}
int32_t main()
{
    int t;
    cin >> t;
    while (t--)
    {
        int numberOfBoxes, numberOfStudents;
        cin >> numberOfBoxes >> numberOfStudents;
        int arr[numberOfBoxes];
        for (int i = 0; i < numberOfBoxes; ++i)
        {
            cin >> arr[i];
        }
        cout << distribute(arr, numberOfBoxes, numberOfStudents) << endl;
    }

    return 0;
}